...
이 강의는 어디까지나 **A/B 검증용 기준 강의**로만 사용한다.
...
- 브랜드/기술 SVG: `Simple Icons`를 1차 소스로 사용
  - 저장소 라이선스는 `CC0-1.0`
  - 다만 프로젝트의 legal disclaimer와 개별 브랜드 사용 맥락은 함께 확인한다
...
5. `icon-coverage-check`를 dev/CI 검사 항목으로 고정
...
### Phase 0.75 — Asset Sourcing / License Lock
...
- `packages/remotion/public/icons/ATTRIBUTION.md`
- `packages/remotion/public/illustrations/ATTRIBUTION.md`

같은 자산 출처 기록 파일을 유지한다.
...
### Phase 3a — shared primitive 작성 + API freeze
...
- benchmark 원본은 별도 동결본으로 분리한다.
- 예:
  - `data/benchmark/lecture-01-03.v0.json`
  - 또는 전용 git tag / commit snapshot

#### 기준 강의

- `lecture-01-03`은 benchmark lecture이자 1차 적용 대상이지만,
  기준선은 반드시 동결 사본으로 따로 보존한다.
...
- `data/benchmark/lecture-01-03.v0.json` 또는 동등한 스냅샷 자산
...
2. 아이콘 감사 + 브랜드 SVG 확장
3. asset sourcing / license 정책 lock + attribution 파일 골격 작성
4. typography / elevation / radius / color token 정비
5. illustration / backdrop 계층 도입
6. semantic motion preset 정비
7. `shared/*` primitive 생성 + preview composition 체크 + API freeze
...
- 3b 종료: 설명형 7종 A/B screenshot
- 3c 종료: 인포그래픽 6종 A/B screenshot
- 3d 종료: 저사용 4종 A/B screenshot
...
### 8.4 Benchmark A/B 비교

`lecture-01-03`은 2개 버전으로 비교한다.

1. 컴포넌트-only 개선 버전
2. 컴포넌트 개선 + JSON 재배치 버전
...
4. `shared/*` primitive가 생성되고 API freeze 체크포인트를 통과함
...
10. benchmark 동결본이 분리되어 있고, A/B mp4 2개 + 리뷰 노트가 준비됨
