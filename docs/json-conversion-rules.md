...
9. **저장 후 자동 검증**
   ...
   - 이어서 `make lint LECTURE=lecture-XX.json STRICT=1` 로 남은 오류를 확인한다 (G-rule 이 syncPoint 커버리지를 검사한다).
   - **Playwright 씬은 녹화 전 `make sync-preview LECTURE=lecture-XX.json SCENE=N` 으로 sync 결과를 사전 시뮬레이션** 한다. 각 액션의 시작 시각 + 그 시점의 narration 발췌가 출력되며, drift > 3 초인 teaching action 은 자동 경고된다. 경고가 있으면 syncPoint 추가/조정 후 재시뮬레이션. webm/Lambda 재실행 없이 0.5 초만에 판정 가능.
...
9. **검증**
   - `make lint-fix LECTURE=lecture-XX.json`
   - `make lint LECTURE=lecture-XX.json STRICT=1` — G-rule 이 syncPoint 커버리지, H-rule 이 `${capture:key}` 정의 검사
   - **`make sync-preview LECTURE=lecture-XX.json SCENE=N`** — 녹화 전 sync 결과 시뮬레이션. drift > 3 초 teaching action 자동 경고. 경고 있으면 syncPoint 추가/조정 후 재시뮬레이션
