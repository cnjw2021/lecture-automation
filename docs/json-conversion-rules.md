...
- before/after still을 렌더링하고 reference slide 비교 대상과 함께 비교 문서에 기록한다.
...
8. 설명 씬의 정보 밀도 표본을 확인한다. 단문 위주 설명 씬이 남아 있으면 단문 허용 예외인지 기록하거나 재변환한다
9. 신규 또는 재변환 표본은 before/after still과 reference 비교 대상을 남긴다
