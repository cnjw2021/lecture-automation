...
8. **CodePen typing 씬 사전 점검** (`#box-html .CodeMirror textarea` / `#box-css .CodeMirror textarea` 사용 씬)
   - `type.key` 안에 `\n` 가 없다 — 있으면 `pressSequentially` 가 무한 hang. 줄별 `type` + `press Enter` 로 분리한다
   - `type.key` 안에 leading whitespace 가 없다 — CodeMirror auto-indent
   - 한 `type.key` 안에 동일 태그명 열기/닫기 페어 사이에 텍스트가 들어 있지 않다 — auto-close-tag race
   - 연속된 `type` / `press Enter` 사이에 `wait 200ms` 가 들어 있다 — 단순 단일 태그(`<a>...</a>`) 연속 입력만으로도 광역 race 가 발생한다 (자동 삽입: `python3 scripts/insert-codepen-settle-waits.py data/lecture-XX.json`)
   - `type` 직전에 `click #box-html .CodeMirror` 가 있다
   - 정상 패턴 참고: `data/lecture-02-01.json`, `data/lecture-02-02.json`

9. **저장 후 자동 검증**
   - JSON 저장 후 `make lint-fix LECTURE=lecture-XX.json` 로 자동 수정 가능 항목을 반영한다.
   - 이어서 `make lint LECTURE=lecture-XX.json STRICT=1` 로 남은 오류를 확인한다. G-playwright-sync-coverage 는 STRICT=1 일 때만 동작 (#141 옵션 A — 휴리스틱 노이즈 격리). 진짜 sync 검증은 sync-preview 가 담당.
   - I-audio-narration-coherence 는 alignment.json 과 narration 정합성을 검사한다. narration 만 수정하고 TTS 재생성을 누락한 경우 stale 경고 (#141 F-4). 4 chars 이상 diff 면 error.
   - J-narration-visual-coherence 는 추상 슬라이드 visual + 화면 의존 표현 mismatch 를 검출한다 (#141 옵션 E). false positive 가능성 있어 사람 판정 필요.
   - **`make run` / `make run-lambda` 는 lint 다음에 `sync-preview-gate` 도 prerequisite 으로 자동 실행** (#141 옵션 B). segment 경고 ≥ 1 이면 차단되므로 별도 sync-preview 호출은 불필요. 수동 점검은 `make sync-preview LECTURE=lecture-XX.json SCENE=N` 으로 0.5 초만에 가능.
   - sync-preview 가 OK 하면 TTS 생성 후 파이프라인 1.7b 또는 `make sync-playwright LECTURE=lecture-XX.json` 로 wait 를 재계산한다. TTS 생성 전 단독 실행은 문자수 기반 추산이므로 최종 sync 로 간주하지 않는다.
   - RecordVisualUseCase 는 capture 직후 manifest 와 audio 길이를 비교해 차이 > 500ms warning, > 1000ms critical 출력 (#141 F-2).
...
6. 코드와 렌더링 결과의 대응(`CodeRenderMappingScreen`), HTML 트리와 화면 결과의 동시 표시(`StructureToRenderScreen`), Flexbox 축/배치(`FlexLayoutDiagramScreen`), 셀렉터와 매칭 요소의 대응(`SelectorMatchScreen`)이 핵심이면 해당 도메인 컴포넌트를 직접 사용한다. CSS 박스 모델은 `BoxModelDiagramScreen`. 위에 해당하지 않는 새 도메인 패턴은 가장 가까운 기존 컴포넌트로 대체하고 후속 이슈로 기록한다.
...
- before/after still을 영속 경로에 저장하고 reference slide 비교 대상과 함께 비교 문서에 기록한다. before 기준은 각 비교 PR의 base SHA 또는 base branch HEAD 시점을 명시한다.
- 비교용 still은 기본 960x540 또는 동등한 절반 해상도로 저장하고, 같은 압축 명령(`pngquant`/`oxipng` 등)을 같은 디렉터리의 모든 PNG에 일괄 적용한다. 원본 1920x1080 PNG는 특별한 픽셀 검수가 필요할 때만 커밋한다.
...
7. **${capture:key} placeholder 사용**
   - `right_click.captureFromTarget` 또는 `capture` 액션의 `saveAs` 로 정의한 키만 후속 씬의 `url` / `selector` / `key` / `html` / `css` / `js` 필드에서 `${capture:key}` 로 참조할 수 있다.
   - placeholder 가 사용되는 위치보다 더 앞 씬 (또는 같은 씬의 더 앞 액션) 에서 saveAs 가 정의되어야 한다. lint H-rule 이 사전 검증.
   - sync-preview / sync-playwright 단계에서 길이 추산을 보정하려면 `tmp/playwright-captures/{lectureId}/.mock.json` 에 mock 값을 둔다.
   - 자세한 명세는 `docs/playwright-actions.md` 의 `right_click` / `capture` 섹션 참조.

8. **CodePen typing 씬 사전 점검** (`#box-html .CodeMirror textarea` 또는 `#box-css .CodeMirror textarea` 를 사용하는 모든 씬)
   - `type.key` 안에 `\n` 가 없다 — 있으면 `pressSequentially` 가 무한 hang 한다. 줄별 `type` + `press Enter` 로 분리한다 (상세: `docs/playwright-conversion-rules.md` `\n` 회피 항목)
   - `type.key` 안에 leading whitespace 가 없다 — CodeMirror 가 auto-indent 한다
   - 한 `type.key` 안에 동일 태그명의 열기/닫기 페어 사이에 텍스트가 들어 있지 않다 (예: `<p><strong>X</strong>Y</p>`) — auto-close-tag race 로 페이지가 close 될 수 있다
   - **연속된 `type` / `press Enter` 사이에 `wait 200ms` 필수** — 단순 단일 태그 연속 입력만으로도 광역 auto-close race 가 발생한다 (실측: lecture-02-03 씬 25). 자동 삽입: `python3 scripts/insert-codepen-settle-waits.py data/lecture-XX.json`
   - `type` 직전에 `click #box-html .CodeMirror` (또는 `#box-css .CodeMirror`) 가 있다

9. **검증**
   - `make lint-fix LECTURE=lecture-XX.json`
   - `make lint LECTURE=lecture-XX.json STRICT=1` — H-rule 이 `${capture:key}` 정의, I-rule 이 audio/narration 정합성, J-rule 이 narration-visual mismatch 검사. G-rule (syncPoint 커버리지 휴리스틱) 은 STRICT=1 일 때만 동작 (#141 옵션 A)
   - **`make run` / `make run-lambda` 가 sync-preview-gate 를 자동 실행** (#141 옵션 B). segment 경고 ≥ 1 면 차단
   - 수동 사전 점검: `make sync-preview LECTURE=lecture-XX.json SCENE=N` — drift > 3 초 teaching action 자동 경고
   - sync-preview OK 후 TTS 생성 → `make sync-playwright LECTURE=lecture-XX.json` 또는 전체 파이프라인 1.7b 단계로 최종 wait 재계산
...
- ❌ CodePen `type.key` 안에 `\n` 사용 — Playwright `pressSequentially` 가 per-character timeout 없이 무한 hang. 다른 액션은 timeout 으로 빠지지만 `type` 만 빠지는 구멍이라 1시간+ 멈춤이 발생한다. 줄별 `type` + `press Enter` 로 반드시 분리. 들여쓰기 leading whitespace 도 `type.key` 에 넣지 말 것 — CodeMirror 가 auto-indent 한다. 정상 패턴: `data/lecture-02-01.json`, `data/lecture-02-02.json` 의 모든 CodePen typing 씬 (상세 분할 예시: `docs/playwright-conversion-rules.md` `\n` 회피 항목)
- ❌ CodePen 연속 `type` / `press Enter` 사이 wait 누락 — auto-close-tag · auto-close-bracket · auto-indent 가 settle 되기 전에 다음 `pressSequentially` 가 시작되면 광역 race 로 hang. 단일 태그(`<a>...</a>`) 연속 입력만으로도 발생한다 (실측: lecture-02-03 씬 25). 모든 연속 `type`/`press Enter` 페어 사이에 `wait 200ms` 필수. 자동 삽입: `python3 scripts/insert-codepen-settle-waits.py data/lecture-XX.json`
...
5. strict lint 로 남은 문제를 확인한다
   ...
   make lint LECTURE=lecture-XX.json STRICT=1
   ```
6. **Playwright 씬은 녹화 전 `make sync-preview LECTURE=lecture-XX.json SCENE=N` 으로 sync 사전 시뮬레이션** — drift > 3 초 teaching action 자동 경고. 경고 있으면 syncPoint 추가 후 재시뮬레이션. webm/Lambda 재실행 없이 0.5 초만에 판정
7. Playwright 씬은 TTS 생성 후 `make sync-playwright LECTURE=lecture-XX.json` 또는 전체 파이프라인 1.7b 단계로 wait 재계산
8. Playwright 씬은 webm 또는 최종 렌더에서 시작부 공백, 후반 무음 타이핑, 잘못된 서술이 없는지 육안 확인
9. 설명 씬의 정보 밀도 표본을 확인한다. 절차와 산출물은 위 "Slide information density policy"를 따른다
10. 신규 또는 재변환 표본은 before/after still과 reference 비교 대상을 영속 경로에 남긴다
11. lint/sync/육안 확인 결과를 JSON에 반영 후 확정 → 다음 강의
