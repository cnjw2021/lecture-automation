...
- CodePen 기본 setup floor 는 `goto 4.2초 + mouse_move 0.8초 + click 0.5초 + 여유 1초` 로 **약 6.5초**다.
- 첫 phrase 가 0~3초에 나오면 구조적으로 맞지 않는다. 도입 나레이션을 "CodePen を開いて準備します" 계열로 확보하고, 실제 타이핑/조작 phrase 를 뒤쪽으로 민다.
- syncPoint 의 `actionIndex` 는 `wait`, `goto`, `wait_for` 가 아니라 실제 teaching action 에 둔다. 예: `type`, `press`, `mouse_move`, `highlight`, 설명 대상인 `click`.

예:
- ❌ `"こんにちは。見出しを入力します"` 직후 `actionIndex: 5` (`wait`) 배치
- ❌ `goto + mouse_move + click` 이전인 0~3초 phrase 에 첫 `type` 을 맞추려 함
- ✅ `"では、CodePen を開いて準備します。HTML の入力欄にカーソルを置いたら、見出しを入力します"` 처럼 setup 설명 후 첫 teaching action phrase 를 둠
...
2. `pre-fill`
   ...
   - 규칙: 나레이션 시작 전, syncPoints 에 포함 금지, 가능한 짧게
...
- ❌ syncPoint 를 `wait`, `goto`, `wait_for` 에 배치 — syncPoint 는 실제 teaching action 에 둔다
...
3. Playwright 씬이 있으면 작성 후 아래를 점검
   ...
   - syncPoint 가 `wait`, `goto`, `wait_for` 가 아니라 teaching action 을 가리키는가
   - 각 syncPoint 세그먼트에서 `type` 포함 고정 액션 시간이 narration budget 을 넘지 않는가
   - pre-fill 은 나레이션 시작 전이며 syncPoints 에 포함되지 않았는가
