...
## Fish Audio API — speech-1.6 (일본어 voice cloning)
...
| 이름 | 소유자 | Voice ID | 톤 |
|---|---|---|---|
| `キラ｜やや落ち着きトーク` | `@Koh Kira` | `bfa248a6698e4986864f70791270032f` | 약간 차분한 강사 톤 |
| `明瞭解説` | `@yuusuke akaike` | `041e23a7181c4e04b3fd4b1846ea696d` | 명료한 해설 톤 |

PR #155 검증 종료 시 채택 보이스 확정 + 강의 적합도 등급 기재 예정.

### 현재 운영 설정

- `modelName`: `speech-1.6` (안정·일관성 우위. `s1` / `s1-mini` 도 선택 가능)
- `temperature`, `topP`: 낮을수록 차분, 변동성 ↓ (강의 나레이션 검증값 **0.1**)
- `speed`: 1.0
- `normalize`: true
