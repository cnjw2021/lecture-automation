...
**연속 `type` / `press Enter` 사이에 settle wait 200ms 필수 (auto-close-tag race 광역 회피)**

위 "중첩 닫기 태그" 룰만 지켜도 race 가 사라질 것 같지만, **실측상 단순 단일 태그(`<a>...</a>`)
연속 입력만으로도 같은 hang 이 발생한다**. CodeMirror 의 auto-close-tag, auto-close-bracket
(따옴표 자동 짝맞춤), auto-indent 이벤트는 모두 비동기로 처리되며, settle 이 끝나기 전에
다음 `pressSequentially` 가 시작되면 같은 race 가 광범위하게 일어난다.

실측: `lecture-02-03` 씬 25 — `<a href="..." target="_blank">Googleを開く</a>` → `press Enter`
→ `<a ...>YouTubeを見る</a>` → `press Enter` → `<img ...>` 사이에 wait 가 없어서 hang
(단순 단일 태그 3개 연속). 같은 패턴인 씬 6/8/15/18 은 같은 실행에서 우연히 통과.
**비결정적**이라 한 번 통과해도 다음에 깨질 수 있다.

규칙 (예외 없음):
- CodePen `.CodeMirror textarea` 를 대상으로 하는 `type` 또는 `press Enter` 가 연속될 때마다
  사이에 `{ "cmd": "wait", "ms": 200 }` 를 **반드시** 끼운다
- 기존 `wait 0` (sync marker) 가 이미 있으면 200 으로 올린다
- "단일 태그라 안전" 이라는 예외를 만들지 않는다 — 비결정성이 광역으로 작용함
- syncPoint 의 actionIndex 가 바뀌므로 같이 갱신한다

자동 삽입 스크립트 (재발 방지·일괄 적용용): `scripts/insert-codepen-settle-waits.py`

설계 형태:

```json
{ "cmd": "type", "selector": "#box-html .CodeMirror textarea", "key": "<a href=\"...\">A</a>" },
{ "cmd": "wait", "ms": 200 },
{ "cmd": "press", "key": "Enter" },
{ "cmd": "wait", "ms": 200 },
{ "cmd": "type", "selector": "#box-html .CodeMirror textarea", "key": "<a href=\"...\">B</a>" },
{ "cmd": "wait", "ms": 200 },
{ "cmd": "press", "key": "Enter" },
{ "cmd": "wait", "ms": 200 },
{ "cmd": "type", "selector": "#box-html .CodeMirror textarea", "key": "<img src=\"...\" alt=\"...\">" }
```

총 액션 시간이 늘어나므로 narration budget 을 다시 점검한다 (`type` × N + 0.2초 × (N-1)).
일반적으로 200ms × 액션 수 추가는 수 초 수준이라 36~84초 narration 에 영향 미미.
