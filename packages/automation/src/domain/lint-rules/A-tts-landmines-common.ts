...
export const COMMON_LANDMINES: Landmine[] = [
  ...
  // C-7: 入れる → "ヒレル" 오독. 動詞 入れる(いれる) 활용형 전체.
  // 입자 れ 가 따라오는 형태는 자동사 入る(はいる)와 충돌하지 않음 (はいる 활용형은 入って/入った 등 れ 없음).
  // ひらがな 변환으로 안전하게 회피. 入れ墨/受け入れ 등 명사·복합어도 동일하게 정규화 가능.
  { pattern: /入れ/g, from: '入れ', to: 'いれ', reason: '入れる 동사 → "ヒレル" 오독. ひらがな 변환' },
];
